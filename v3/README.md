...
当前开发版本：**0.4.1**（三网关并行资源探测、共享 5 秒预算、本地缓存快速通道、Supabase 主通道、Cloudflare 备用、无客户端 Git 依赖、按需流式下载、
授权租约保护、覆盖层快速待命、LCU 阶段同步和 WAD 槽位冲突修复）。

私有仓库架构、部署和验证记录见 [`修复文档汇总.md`](../修复文档汇总.md)。
...
从 0.4.0 开始，程序启动前会有限检查远程版本清单。只有服务端明确返回高于当前版本的 `minimumVersion` 时才进入强制更新页面；页面提供蓝奏云下载地址、提取码和 QQ 群备用入口。版本服务暂时不可用时会写入日志并继续本次启动，避免网络瞬态故障锁死现有用户；下次启动会再次检查。
...
中国大陆用户可能无法稳定访问普通 `workers.dev` 域名。当前客户端优先使用 Supabase Tokyo，然后使用 `https://license.re2347.ccwu.cc`，最后才尝试 `workers.dev`。客户端只在连接失败、超时、明确配额/限流错误、408、429 或 5xx 时切换；库存、绑定、租约、撤销、过期、签名和其他业务错误都不会被备用端点覆盖。详细规则见 [`修复文档汇总.md`](../修复文档汇总.md)。
