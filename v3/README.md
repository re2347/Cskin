...
当前开发版本：**0.3.0**（私有资源 Worker、无客户端 Git 依赖、按需流式下载、
...
3. 按拼音浏览或搜索英雄，选中皮肤后点击“应用皮肤”。首次启动先使用随包索引，授权通过后由 Worker 更新私有索引并按需下载皮肤。

程序启动时会检查私有资源 Worker，运行期间每 10 分钟自动检查一次；仓库新增或修正皮肤会合并到当前列表和本地引擎索引。网络暂时不可用时会继续使用上一次成功同步的索引和已经校验的本地皮肤缓存。
...
`.fantome` 都由授权 Worker 返回。GitCode 令牌只配置为 Cloudflare Worker Secret。
...
安装器输出到 `installer-output\CskinSetup.exe`，默认用户级安装到 `%LOCALAPPDATA%\Programs\PortableCskin`。安装器不需要管理员权限；程序文件与运行数据分离，运行数据固定保存在 `%LOCALAPPDATA%\PortableCskin`，因此更换安装目录、版本更新不会丢失授权。
...
程序文件可以放在任意目录；授权和运行数据根固定为 `%LOCALAPPDATA%\PortableCskin`。旧版本程序目录中的 `authorization` 会在首次启动时自动导入，目录中的主要内容如下：
...
首次运行后产生的缓存和授权信息都在 `%LOCALAPPDATA%\PortableCskin`。更换安装位置或升级版本无需复制授权文件；更换硬盘后，只要主板信息保持不变，输入原卡密即可受控恢复设备密钥。更换主板、Windows 用户或手动删除授权目录时，需要管理员重置设备绑定。

资源同步只使用 HTTPS Worker API，不运行 `git clone`、`git fetch`、`git reset` 或 `git cat-file`。

应用不会将完整皮肤库打包进安装文件；只会在选择皮肤时按 `skins/<championId>/.../<skinId>.fantome` 精确缓存。

运行数据保存在 `%LOCALAPPDATA%\PortableCskin`：索引缓存位于 `Assets`，皮肤文件位于 `Engine\skins`，引擎运行目录位于 `Engine`。程序不依赖开发机的用户目录或下载目录。

顶部的“退出并清理”会在确认后停止本地引擎，删除 `%LOCALAPPDATA%\PortableCskin` 中的引擎数据和已下载皮肤，但保留授权、设置和私有索引缓存。普通关闭按钮不会自动删除资源。
...
中国大陆用户可能无法稳定访问普通 `workers.dev` 域名。当前客户端使用 `https://license.re2347.ccwu.cc` 作为主端点，并保留 `workers.dev` 作为备用；正式面向大陆发布时，建议再部署一个已备案的中国大陆 HTTPS 中继并加入 `Services/AuthorizationClient.cs`。客户端只在连接失败、超时、408、429 或 5xx 时切换端点；授权业务错误不会被备用端点覆盖。Cloudflare China Network 需要 Enterprise、合作伙伴和备案条件，普通账号不能直接开启。详细的中继安全要求和上线检查见 [`CHINA_AUTH_CONNECTIVITY.md`](CHINA_AUTH_CONNECTIVITY.md)。

授权窗口提供“记住密钥”选项。密钥使用 Windows DPAPI（当前用户范围）加密后保存到 `%LOCALAPPDATA%\\PortableCskin\\authorization\\remembered-license.bin`。只有明确勾选才会在更新或重装后自动恢复；未勾选时不会因残留租约自动进入。进入主界面后，顶部会显示授权到期时间和剩余天数/小时数。
...
## Supabase 备用认证方案（本地准备，未上线）

仓库中的 `supabase/` 已准备 Tokyo 项目 `kzqgphxrghdclkwneqyn` 的 PostgreSQL schema 和 Edge Function。Cloudflare Worker + D1 仍是客户端默认认证端点；显式设置 `CSKIN_AUTH_PROVIDER=supabase` 后，客户端按 `Supabase → Cloudflare 自定义域名 → workers.dev` 自动故障转移。只有连接失败、超时、无效响应、408、429 或 5xx 才切换；401/403 和许可证业务错误不会被备用端点覆盖。缺少的 `SUPABASE_SERVICE_ROLE_KEY`、数据库密码和 `LICENSE_PEPPER` 必须只通过本地/平台密钥注入。
